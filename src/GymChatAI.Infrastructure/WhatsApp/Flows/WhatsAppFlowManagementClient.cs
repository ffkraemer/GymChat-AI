...
/// definition, set its endpoint URI, publish, and check status - plus the one-time
/// per-phone-number encryption key registration. Lets all of this happen from our
...
public class WhatsAppFlowManagementClient : IWhatsAppFlowManagementClient
{
        ...
        string whatsAppBusinessAccountId, string name, IReadOnlyList<string> categories, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError("Failed to create WhatsApp Flow '{Name}': {StatusCode} {Body}", name, response.StatusCode, body);
            throw new InvalidOperationException($"Meta rejected the create-flow request ({response.StatusCode}): {body}");
        }

        var result = JsonSerializer.Deserialize<CreateFlowResponse>(body);
        if (string.IsNullOrWhiteSpace(result?.Id))
            throw new InvalidOperationException($"WhatsApp API did not return a Flow id. Raw response: {body}");
    ...
    public async Task<UpdateFlowJsonResult> UpdateFlowJsonAsync(string metaFlowId, string flowJson, CancellationToken cancellationToken = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            return new UpdateFlowJsonResult(false, [new WhatsAppFlowValidationError("HTTP_ERROR", body)]);
        }
        ...
        var result = JsonSerializer.Deserialize<UpdateFlowJsonResponse>(body);
        var validationErrors = (result?.ValidationErrors ?? [])
            .Select(e => new WhatsAppFlowValidationError(e.Error, e.Message))
            .ToList();
    ...
    public async Task<bool> SetFlowEndpointAsync(string metaFlowId, string endpointUri, CancellationToken cancellationToken = default)
    {
        var payload = new { endpoint_uri = endpointUri };
        var response = await _httpClient.PostAsJsonAsync($"{metaFlowId}", payload, cancellationToken);
        if (response.IsSuccessStatusCode) return true;

        var body = await response.Content.ReadAsStringAsync(cancellationToken);
        _logger.LogWarning("Failed to set endpoint URI for Flow {MetaFlowId}: {StatusCode} {Body}", metaFlowId, response.StatusCode, body);
        return false;
    }

    public async Task<(bool Success, string? ErrorMessage)> PublishFlowAsync(string metaFlowId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<bool> DeleteFlowAsync(string metaFlowId, CancellationToken cancellationToken = default)
    {
        var response = await _httpClient.DeleteAsync($"{metaFlowId}", cancellationToken);
        if (response.IsSuccessStatusCode) return true;

        var body = await response.Content.ReadAsStringAsync(cancellationToken);
        _logger.LogWarning("Failed to delete Flow {MetaFlowId}: {StatusCode} {Body}", metaFlowId, response.StatusCode, body);
        return false;
    }
}
